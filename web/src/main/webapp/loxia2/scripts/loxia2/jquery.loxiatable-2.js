(function($){
    ...
    var loxiaSimpleTable = {
        ...
        _resetTable: function(data){
            ...
            if(dataflag){
                ...
                if(!this.option("currentCount") && this.option("nodatamessage"))
                	...
                	if(this.option("groupedColumns")){
                		// ADDED BY fan.chen, group table row by some columns
                		// require underscore
                		tbody += this._drawGroupedBody(_d, this.option("groupedColumns"));
                	}else{
        ...
        // draw grouped body
        _drawGroupedBody: function(datas, columns){
        	var rows = "";
        	var self = this;
        	_.each(_.groupBy(datas, function(ele){
        		return self._groupLinesKey(ele, columns);
        	}), function(groupedData, i){
        		rows += self._drawGroupLines(groupedData, i);
        	});
        ...
        // draw grouped rows
        _drawGroupLines: function(rows){
        	
            var rowsString = "";
            var isRowSpaned = false;
            var rowSpan = rows.length;
            for(var i = 0; i < rowSpan; i++){
            	var row = rows[i];
            	rowsString += this._drawGroupedLine(row, i, isRowSpaned, rowSpan);
            	isRowSpaned = true;
            }
            return rowsString;
        },
        
        _drawGroupedLine: function(rowData, rowIndexInGroup, isRowSpaned, rowSpan){
        	var cols = this.option("cols");
        	var tr = "<tr class='" + (rowIndexInGroup%2==0?"odd":"even") + "'>";
        	for(var i = 0; i < cols.length; i++){
        		var currentColumnName = cols[i].name;
        		if(_.contains(this.option("groupedColumns"), currentColumnName)){
        			if(!isRowSpaned){
        				tr += "<td class='col-" + i + " " + (cols[i].align? cols[i].align : "") + "' rowspan='" + rowSpan + "'>";
        				tr += this._getData(rowData, cols[i], i);
        			}
        		}else{
        			tr += "<td class='col-" + i +" " + (cols[i].align? cols[i].align :"")+ "'" +
                    ">" + this._getData(rowData,cols[i],i) + "</td>";
        		}
        	}
        	return tr;
        },
        
        // join columns value to one string, if equals, they should be in one group
        _groupLinesKey: function(lineData, columns){
        	var str = "";
        	for(var i = 0; i < columns.length; i++){
        		str += lineData[columns[i]];
        	}
        	return str;
        },
